...
# Design Compiler synthesis script for Alarm Clock
...
set DESIGN_NAME alarm_clock_top
...
###############################################################################
# Neutralize accidental power-budget constraints since the default is zero
###############################################################################
...
proc ac_disable_power_budget_constraints {} {
    puts "INFO: Setting non-limiting power budgets"
        ...
        puts "INFO: Current max_leakage_power = [get_attribute [current_design] max_leakage_power]"
    }
}
...
set RTL_FILES [list \
    $SOURCE_DIR/timegen.v \
    $SOURCE_DIR/counter.v \
    $SOURCE_DIR/alarm_reg.v \
    $SOURCE_DIR/keyreg.v \
    $SOURCE_DIR/fsm.v \
    $SOURCE_DIR/lcd_driver.v \
    $SOURCE_DIR/lcd_driver_4.v \
    $SOURCE_DIR/alarm_clock_top.v \
]
...
require_file $SOURCE_DIR/alarm_clock.sdc
...
source $SOURCE_DIR/alarm_clock.sdc

# Neutralize accidental max_leakage_power=0 / max_dynamic_power=0 constraints.
...
###############################################################################
# Flatten display hierarchy only
###############################################################################

if {[sizeof_collection [get_cells -quiet lcd_disp]] > 0} {
    ungroup -flatten [get_cells lcd_disp]
}
...
write -hierarchy -format verilog -output $RESULT_DIR/alarm_clock_compiled.v
write -hierarchy -format ddc     -output $RESULT_DIR/alarm_clock_compiled.ddc
write_sdc $RESULT_DIR/alarm_clock_out.sdc

###############################################################################
# Strip power-budget commands from ICC2 output SDC if written
###############################################################################

set clean_sdc $RESULT_DIR/alarm_clock_out.sdc
...
file copy -force $RESULT_DIR/alarm_clock_compiled.v ../../icc2/source/alarm_clock_compiled.v
file copy -force $RESULT_DIR/alarm_clock_out.sdc    ../../icc2/source/alarm_clock_out.sdc
